...
def sum_zero(list1):
    ...
    for item in new_list:
        for each_item in new_list[new_list.index(item):]:
            if item + each_item == 0:
                final_list+= [[item, each_item]]
...
def word_length(words):
    ...
    for each_word in words:
        ...
        if word_length in word_length_dict:
            word_length_dict[word_length] = word_length_dict[word_length] + [each_word]
        else:
            word_length_dict[word_length] = [each_word]

    word_length_tuple = word_length_dict.items()
        

    return word_length_tuple
...
def adv_word_length_sorted_words(words):
    ...
    for v in word_length_dict.values():
        v = v.sort()
...
def pirate_talk(phrase):
    ...
    pirate_dict = {"sir" : "matey", "hotel": "fleabay inn", "student": "swabbie", "boy": "matey",
                    "madam": "proud beauty", "professor": "foul blaggart", "restaurant": "galley", "your" : "yer",
                    "excuse": "arr", "students": "swabbies", "are" : "be", "lawyer": "foul blaggart", "the": "th\'",
    ...
    for word in word_list:
        if word in pirate_dict:
            pirate_list.append(pirate_dict[word])
